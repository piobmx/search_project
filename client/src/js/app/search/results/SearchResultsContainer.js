...
const getState = function () {
    return {
        ...
        elapsedTime: (SearchStore.getElapsedTime() / 1000)
            .toFixed(2)
            .toString(),
...
export default class SearchResultsContainer extends React.Component {
    ...
    _onChange() {
        ...
        this.state.results.forEach(
            (result) => (resultIdMap[Helpers.getId(result)] = true)
        );
        
        const newResults = newState.results.filter(
            (result) => !resultIdMap[Helpers.getId(result)]
        );
        const newCollapsibleResults = newResults.filter(this.isCollapsible);
        const newCollapsibleResultIds = newCollapsibleResults.map(
            (result) => result.id
        );
        if (newCollapsibleResultIds.length > 0) {
            this.hideCollapsedResults(newCollapsibleResultIds);
        }
        if (
            newState.searchState.page !== this.state.searchState.page ||
            newState.searchState.query !== this.state.searchState.query
        ) {
    ...
    getMetaInfo() {
        return {
            bookmarkedIds: this.state.results
                .filter((result) => result.metadata.bookmark)
                .map((result) => Helpers.getId(result)),
            excludedIds: this.state.results
                .filter((result) => result.metadata.exclude)
                .map((result) => Helpers.getId(result)),
    ...
    isCollapsible(result) {
        return (
            this.state.distributionOfLabour &&
            !this.state.tutorial &&
